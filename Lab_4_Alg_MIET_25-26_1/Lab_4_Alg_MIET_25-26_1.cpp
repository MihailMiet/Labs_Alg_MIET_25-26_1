...
int countLiveNeighbors(int field[SIZE][SIZE], int row, int col) {//f подсчета живых соседей (с учетом зацикленности поля размером SIZExSIZE)
    ...
    // Проходим по 8 соседям (от -1 до +1 по строкам и столбцам)
    for (int i = -1; i <= 1; ++i) {
        for (int j = -1; j <= 1; ++j) {
            if (i == 0 && j == 0) continue;// Пропускаем саму себя
...
int main() {
    ...
    //Пункт 1: Вывод изначального состояния
